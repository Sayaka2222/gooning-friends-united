...
NGame = {
	...
	LAG_DAYS_FOR_LOWER_SPEED = 500,					-- Days of client lag for decrease of gamespeed
	LAG_DAYS_FOR_PAUSE = 100,						-- Days of client lag for pause of gamespeed.
	GAME_SPEED_SECONDS = { 600.0, 0.5, 0.22, 0.07, 0.0 }, -- game speeds for each level. Must be 5 entries with last one 0 for unbound
	...
	INFO_MESSAGE_TIMEOUT_DAYS = 1,					-- Same but for unimportant messages.
	...
	MISSION_REMOVE_FROM_INTERFACE_DEFAULT = 7,		-- Default days before a mission is removed from the interface after having failed or completed
...
NDiplomacy = {
	...
	TRUCE_BREAK_COST_PP = 999,						-- Base cost in PP of breaking a truce by joining a war or accepting a call to war ( you cannot declare war yourself until the truce if up ), this is then multiplied by the time left on the truce ( so once half the truce is up it only cost 50% of this )
	...
	BASE_BOOST_PARTY_POPULARITY_DAILY_PP = 999, 	-- Daily pp cost for boost party popularity
	...
	BASE_STAGE_COUP_TOTAL_COST = 99999, 				-- Equipment consume factor for stage coup.
	...
	LL_TO_OVERLORD_AUTONOMY_DAILY_FACTOR = 0.0,	-- If puppet lend leases equipment to overlord of at least same tech level as they have, they gain autonomy
	...
	AUTONOMY_LEVEL_CHANGE_PP_COST_BASE = 999,		-- Base cost of changing level of autonomy
	...
	MASTER_BUILD_AUTONOMY_FACTOR = 0.0,            -- scales autonomy gain from construction by this
...
NCountry = {
	...
	REINFORCEMENT_EQUIPMENT_DELIVERY_SPEED = 0.7,	-- Modifier for army equipment reinforcement speed
	...
	INTEL_FROM_ALLIANCE_FACTOR = 1,				-- Multiplied to the difference between a country intel and the maximum value in the alliance to compute the amount of intel that flows from the alliance to that country. 0 means no alliance contribution, 1 means a country intel's is the same as the max in the alliance.
	...
	BASE_MOBILIZATION_SPEED = 0.2,					-- Base speed of manpower mobilization  #in 1/1000 of 1 %
	...
	PROPAGANDA_STABILITY_DAILY_DECAY = 0.0,		-- Amount of stability recovered daily from propaganda effort
	PROPAGANDA_WAR_SUPPORT_DAILY_DECAY = 0.0,		-- Amount of war support recovered daily from war support effort
	...
	AIR_SUPPLY_CONVERSION_SCALE = 0.3,				-- Conversion scale for planes to air supply
	AIR_SUPPLY_DROP_EXPIRATION_HOURS = 168,			-- Air drop length after being dropped
	STARTING_COMMAND_POWER = 100,					-- starting command power for every country
	BASE_MAX_COMMAND_POWER = 200.0,					-- base value for maximum command power
	BASE_COMMAND_POWER_GAIN = 1,					-- base value for daily command power gain
	...
	ATTACHE_XP_SHARE = 0.0,						-- Country received xp from attaches
	...
	BASE_FUEL_GAIN = 10.0,							-- base amount of fuel gained hourly, independent of excess oil
	BASE_FUEL_CAPACITY = 50000,						-- base amount of fuel capacity

	SCORCHED_EARTH_STATE_COST = 9999,					-- pp cost to scorch a state
	...
	GIE_ESCAPING_DIVISIONS_TRANSFER_DAYS = 1, -- days to transfer escaping divisions to host nation
...
NProduction = {
	...
	BASE_LICENSE_IC_COST = 0.4,							-- Base IC cost for lended license
	...
	MIN_NAVAL_EQUIPMENT_CONVERSION_RESOURCE_COST_FACTOR = 1,	-- Minimum fraction of a naval equipment's strategic resource cost that any conversion will cost.
...
NBuildings = {
	ANTI_AIR_SUPERIORITY_MULT = 25.0,	-- How much air superiority reduction to the enemy does our AA guns? Normally each building level = -1 reduction. With this multiplier.
	MAX_BUILDING_LEVELS = 99,			-- Max levels a building can have.
	...
	NAVALBASE_REPAIR_MULT = 0.1,		-- Each level of navalbase building repairs X strength and can repair as many ships as its level
...
NDeployment = {
	BASE_DEPLOYMENT_TRAINING = 2.5,		-- Base training done each day during deployment.
},
...
NMilitary = {
	...
	MAX_LEADERS_TO_SHOW = 20,							--Max officers to show in field officers list, sorted by field EXP. Divisions with awardable entries will potentially supercede this limit
	...
	ENGAGEMENT_WIDTH_PER_WIDTH = 1.0,	-- how much enemy combat width we are allowed to engage per width of our own
	...
	VPS_FOR_HIGH_HISTORY_ENTRY = 800,				-- VPs required for high-level history entry
	...
	CASUALTY_COUNT_FOR_HISTORY_ENTRY = 15000,	-- number of received casualties to receive a history entry (one only)
	...
	MAX_DIVISION_SUPPORT_HEIGHT = 8,		-- Max height of support in division designer.
	...
	LAND_COMBAT_STR_DAMAGE_MODIFIER = 0.037,       -- global damage modifier... but some equipment is returned at end of battles see : EQUIPMENT_COMBAT_LOSS_FACTOR
	...
	ATTRITION_EQUIPMENT_LOSS_CHANCE = 0.015,		   -- Chance for loosing equipment when suffer attrition. Scaled up the stronger attrition is. Then scaled down by equipment reliability.
	ATTRITION_EQUIPMENT_PER_TYPE_LOSS_CHANCE = 1, -- Chance for loosing equipment when suffer attrition. Scaled up the stronger attrition is. Then scaled down by equipment reliability.
	...
	RELIABILTY_RECOVERY = 0.0,                     -- factor affecting how much equipment is returned "from the dead"
	...
	LAND_SPEED_MODIFIER = 0.042,                    -- basic speed control
	...
	UNIT_EXPERIENCE_PER_TRAINING_DAY = 0.0000,
	TRAINING_MAX_LEVEL = 2,
	DEPLOY_TRAINING_MAX_LEVEL = 1,
	TRAINING_EXPERIENCE_SCALE = 62.0,
	TRAINING_ORG = 0.2,
	ARMY_EXP_BASE_LEVEL = 1,
	UNIT_EXP_LEVELS = { 0.2, 0.3, 0.4, 0.6 },		-- Experience needed to progress to the next level
	FIELD_EXPERIENCE_SCALE = 0.005,
	...
	PLAN_PROVINCE_LOW_VP_IMPORTANCE_AREA = 1.0,     -- Used when calculating the value of defense area in the battle plan system
	PLAN_PROVINCE_MEDIUM_VP_IMPORTANCE_AREA = 1.0,  -- Used when calculating the value of defense area in the battle plan system
	PLAN_PROVINCE_HIGH_VP_IMPORTANCE_AREA = 1.0,   -- Used when calculating the value of defense area in the battle plan system
	PLAN_PROVINCE_CAPITAL_IMPORTANCE_AREA = 1.0,	-- Used when calculating the balue of defense area in the battle plan system
	...
	PLAN_PROVINCE_MEDIUM_VP_IMPORTANCE_FRONT = 1, -- Used when calculating the calue of fronts in the battle plan system
	PLAN_PROVINCE_HIGH_VP_IMPORTANCE_FRONT = 1,  -- Used when calculating the calue of fronts in the battle plan system

	PLAN_SHARED_FRONT_PROV_IMPORTANCE_FACTOR = 0.5,	-- If fornt orders share end provinces, they should each have a somewhat reduced prio due to it being shared.

	PLAN_PORVINCE_PORT_BASE_IMPORTANCE = 1.0,		-- Added importance for area defense province with a port
	PLAN_PORVINCE_PORT_LEVEL_FACTOR = 1,			-- Bonus factor for port level
	...
	PLAN_PORVINCE_AIRFIELD_LEVEL_FACTOR = 1,		-- Bonus factor for airfield level
	PLAN_PORVINCE_RESISTANCE_BASE_IMPORTANCE = 1.0, -- Used when calculating the calue of defense area provinces for the battle plan system (factored by resistance level)
	...
	PLAN_PROVINCE_PRIO_DISTRIBUTION_MIN = 1,		-- Lowest fraction of divisions that will be distributed based on province priority
	...
	PLAN_PROVINCE_PRIO_DISTRIBUTION_DPP_HIGH = 1.0, -- At what divisions per province should we use PLAN_PROVINCE_PRIO_DISTRIBUTION_MIN
	PLAN_PROVINCE_PRIO_DISTRIBUTION_DPP_LOW = 1.0,	-- At what divisions per province should we use PLAN_PROVINCE_PRIO_DISTRIBUTION_MAX
	...
	COMBAT_SUPPLY_LACK_ATTACKER_ATTACK = -0.2,     -- attack combat penalty for attacker if out of supply
	COMBAT_SUPPLY_LACK_ATTACKER_DEFEND = -0.7,     -- defend combat penalty for attacker if out of supply
	COMBAT_SUPPLY_LACK_DEFENDER_ATTACK = -0.5,     -- attack combat penalty for defender if out of supply
	...
	UNIT_LEADER_ASSIGN_TRAIT_COST = 0.1,					-- cost to assign a new trait to a unit leader
	...
	ARMY_FUEL_COST_MULT = 0.1,						-- fuel cost multiplier for all army related stuff
	...
	DAMAGE_SPLIT_ON_FIRST_TARGET = 0.9,			--% of damage dealt to the first target in a combat. The rest will be split amongst subsequent targets. Modifiers can affect this up to a maximum of 0.9. That value must not be exposed as a define.
...
NAir = {
	...
	COMBAT_DAMAGE_STATS_MULTILPIER = 1,
	COMBAT_BETTER_AGILITY_DAMAGE_REDUCTION = 0.6, 		-- How much the better agility (than opponent's) can reduce their damage to us.
	COMBAT_BETTER_SPEED_DAMAGE_INCREASE = 0.6, 		-- How much the better Speed (than opponent's) can reduce increase our damage to them.
	...
	COMBAT_MULTIPLANE_CAP = 2.1,						-- How many planes can shoot at each plane on other side ( if there are 100 planes we are atttacking COMBAT_MULTIPLANE_CAP * 100 of our planes can shoot )
	...
	NAVAL_STRIKE_DAMAGE_TO_STR = 0.75,					-- Balancing value to convert damage ( naval_strike_attack * hits ) to Strength reduction.
	NAVAL_STRIKE_DAMAGE_TO_ORG = 1.0,					-- Balancing value to convert damage ( naval_strike_attack * hits ) to Organisation reduction.
	NAVAL_STRIKE_CARRIER_MULTIPLIER = 7.5,              -- damage bonus when planes are in naval combat where their carrier is present (and can thus sortie faster and more effectively)
	...
	ACCIDENT_CHANCE_BASE = 0.0,							-- Base chance % (0 - 100) for accident to happen. Reduced with higher reliability stat.
	ACCIDENT_CHANCE_CARRIER_MULT = 0.0,					-- The total accident chance is scaled up when it happens on the carrier ship.
	ACCIDENT_CHANCE_BALANCE_MULT = 0.0,					-- Multiplier for balancing how often the air accident really happens. The higher mult, the more often.
	ACCIDENT_EFFECT_MULT = 0.0,						-- Multiplier for balancing the effect of accidents
	...
	ACE_DEATH_BY_OTHER_ACE_CHANCE = 0.0,				-- chance to an ace dying by another ace if it was hit by ace in combat
	ACE_DEATH_CHANCE_PLANES_MULT = 0.0,				-- The more airplanes was lost in a single airplanes (more bloody it was) the higher chance of Ace to die.
	AIR_AGILITY_TO_NAVAL_STRIKE_AGILITY = 0.02,         		-- conversion factor to bring agility in line with ship AA
	ACE_EARN_CHANCE_BASE = 0.04,						-- Base chance % for ace pilot creation roll to happen. Happens only when successfully kill airplane/ship or damage the buildings.
	...
	CAPACITY_PENALTY = 0.869,								-- scales penalty of having overcrowded bases.
	...
	AIR_MORE_GROUND_CREWS_COST = 999,					-- CP cost to maintain more ground crews
	...
	EFFICIENCY_REGION_CHANGE_DAILY_GAIN_NAVAL_BOMBER = 0.152,		-- How much efficiency to regain per day. Gain applied hourly.
	EFFICIENCY_REGION_CHANGE_DAILY_GAIN_TACTICAL_BOMBER = 0.152,	-- How much efficiency to regain per day. Gain applied hourly.
	...
	MIN_PLANE_COUNT_PARADROP = 99999,
	...
	MISSION_COMMAND_POWER_COSTS = {  -- command power cost per plane to create a mission
		...
		0.0, -- DROP_NUKE
		0.0, -- PARADROP
		999.0, -- NAVAL_KAMIKAZE
        0.0, -- PORT_STRIKE
		999.0, -- ATTACK_LOGISTICS
		0.05, -- AIR_SUPPLY
		0.0, -- TRAINING
		999.0, -- NAVAL_MINES_PLANTING
		999.0, -- NAVAL_MINES_SWEEPING
		0.0, -- RECON
		0.0, -- NAVAL_PATROL
	},
	...
	FUEL_COST_MULT = 0.05, -- fuel multiplier for all air missions

	MISSION_EFFICIENCY_MULT_AT_LACK_OF_FUEL = 0.25, 				-- multiplier for mission efficiency when a base lacks fuel

	BOMBING_TARGETING_RANDOM_FACTOR = 0.5,							-- % of picking the wrong target
...
NNavy = {
	...
	COMBAT_DAMAGE_TO_STR_FACTOR = 0.2,								-- casting damage value to ship strength multiplier. Use it ot balance the game difficulty.
	...
	COMBAT_ARMOR_PIERCING_DAMAGE_REDUCTION = 0.9,					-- All damage reduction % when target armor is >= then shooter armor piercing. (depricated)
	...
	NAVAL_COMBAT_RESULT_TIMEOUT_YEARS = 5,							-- after that many years, we clear the naval combat results, so they don't get stuck forever in the memory.
	...
	CAPITAL_ONLY_COMBAT_ACTIVATE_TIME = 16,                          -- hours from start of combat when only carriers, capitals and subs get to attack
	ALL_SHIPS_ACTIVATE_TIME = 20,                                    -- hours where all get to attack
	...
	AGGRESSION_LIGHT_GUN_EFFICIENCY_ON_LIGHT_SHIPS = 1.2, -- ratio for scoring for different gun types against light ships
	...
	AGGRESSION_HEAVY_GUN_EFFICIENCY_ON_HEAVY_SHIPS = 1.2, -- ratio for scoring for different gun types against heavy ships
	...
	TRAINING_ACCIDENT_STRENGTH_LOSS_FACTOR = 0.0,						-- Amount of strength loss in a training accident, propotional to the maximum strength of the ship
	TRAINING_ACCIDENT_ORG_LOSS_FACTOR = 0.0,						-- Amount of current organization the ship lose

	ACCIDENTS_CHANCE_BALANCE_FACTOR = 0.0,							-- General chance for naval accidents for balancing the gameplay.
	...
	TRAINING_MAX_DAILY_COUNTRY_EXP = 0.08,							-- Maximum navy XP daily gain
	...
	BASE_SPOTTING = 2,												-- base spotting percentage for navy
	...
	ESCAPE_SPEED_SUB_BASE = 0.06,									-- subs get faster escape speed. gets replaced by hidden version below if hidden
	ESCAPE_SPEED_HIDDEN_SUB = 0.12,									-- hidden subs get faster escape speed
	...
	SUB_DETECTION_CHANCE_BASE_SPOTTING_EFFECT = 1.0,				-- effect of base spotting for initial spotting of pure submarine forces. this along with next value is added together and rolled against a random to start spotting
	...
	UNIT_TRANSFER_SPOTTING_SPEED_MULT = 2.5,						-- spotting speed mult against unit transfers
	...
	POSITIONING_PENALTY_FOR_SHIPS_JOINED_COMBAT_AFTER_IT_STARTS		= 0.0, -- each ship that joins the combat will have this penalty to be added into positioning
	MAX_POSITIONING_PENALTY_FOR_NEWLY_JOINED_SHIPS 					= 0.0,  -- the accumulated penalty from new ships will be clamped to this value
	POSITIONING_PENALTY_HOURLY_DECAY_FOR_NEWLY_JOINED_SHIPS			= 0.0,-- the accumulated penalty from new ships will decay hourly by this value
	...
	SHIP_TO_FLEET_ANTI_AIR_RATIO									= 0.1,	-- total sum of fleet's anti air will be multiplied with this ratio and added to calculations anti-air of individual ships while air damage reduction
	...
	MAX_ANTI_AIR_REDUCTION_EFFECT_ON_INCOMING_AIR_DAMAGE 			= 0.7,	-- damage reduction for incoming air attacks is clamped to this value at maximum.
	...
	HIT_PROFILE_SPEED_FACTOR										= 0.75,		-- factors speed value when determining it profile (Vis * HIT_PROFILE_MULT * Ship Hit Profile Mult)
	...
	SUBMARINE_REVEAL_BASE_CHANCE 									= 25,		-- Base factor for submarine detection. It's modified by the difference of a spotter's submarines detection vs submarine visibility. Use this variable for game balancing. setting this too low will cause bad spotting issues.
	SUBMARINE_REVEAL_POW 											= 2.0,		-- A scaling factor that is applied to the reveal chance in order to make large differences in detection vs visibility more pronounced
	...
	NAVAL_COMBAT_AIR_CARRIER_TARGET_SCORE = 30,
...
NAI = {
	...
	PLAN_FRONTUNIT_DISTANCE_FACTOR = 30.0,		-- Factor for candidate units distance to front positions.
	...
	CONSTRUCTION_PRIO_FACTOR_REPAIRING = 10,                                  -- factor prio with this if building is being repaired
...
NOperatives = {
	...
	AGENCY_UPGRADE_PER_OPERATIVE_SLOT = 1,			-- Number of upgrade needed to unlock an additional operative slot
	MAX_OPERATIVE_SLOT_FROM_AGENCY_UPGRADES = 1,	-- max operative slots gained from upgrades
	AGENCY_OPERATIVE_RECRUITMENT_TIME = 15,			-- Number of days to wait to have operative to recruit when an operative slot first becomes available
	...
	BASE_COUNTER_INTELLIGENCE_RATING = 1,					-- Base national counter intelligence rating for all countries
	...
	INTEL_NETWORK_MIN_VP_TO_TARGET = 1,					-- The minimum value of the highest VP in a state to consider the state as a valid target to start building an intel network
	INTEL_NETWORK_MIN_STRENGTH_TO_TARGET = 1.0,				-- The minimum value of the intel network in a state to consider it a valid target to deploy an operative in
	...
	BUILD_INTEL_NETWORK_DAILY_XP_GAIN = 3,
	...
	COUNTER_INTELLIGENCE_DAILY_XP_GAIN = 1.5,
	...
	ROOT_OUT_RESISTANCE_DAILY_XP_GAIN = 1.5,
	...
	OPERATIVE_MAX_DAYS_TO_AUTO_RESUME_MISSION = 121,				-- Maximum number of days an operative has to be disabled before its mission is not automatically resumed once he is available again
	MAX_RECRUITED_OPERATIVES = 10,

	CRYPTO_BASE_CRYPTO_LEVEL = 4000,						-- base crypto strength for a country
	CRYPTO_CRYPTO_LEVEL_PER_CRYPTO_UPGRADE = 1500,			-- crypto strength per crypto upgrade
	...
	ON_CAPTURE_COUNTERINTELLIGENCE_OPERATIVE_XP_GAIN = 300,					-- Xp gain when an enemy operative is captured in the country the operative is assigned to counter intelligence to. Apply to a single randomly selected operative
...
NIntel = {
	...
	DYNAMIC_INTEL_SOURCE_EVENT_MAXIMUMS = { 200, 200, 200, 200 },
	DYNAMIC_INTEL_SOURCE_EVENT_ABSOLUTE_MAXIMUMS = { 200, 200, 200, 200 },
...
NCharacter = {
	...
	DEFAULT_PP_COST_FOR_MILITARY_ADVISOR = 200,	-- When an advisor does not have cost assigned this is the default used
	...
	ADVISOR_PROMOTION_COST = 9999,	-- Cost to promote someone to advisor
...
NSupply = {
	...
	CAPITAL_STARTING_PENALTY_PER_PROVINCE = 0.4, -- starting penalty that will be added as supply moves away from its origin (modified by stuff like terrain)
	...
	NODE_STARTING_PENALTY_PER_PROVINCE = 0.70,
	...
	FLOATING_HARBOR_BASE_DURATION = 48, -- duration of a full hp floating harbor
	FLOATING_HARBOR_DURATION_RATIO_AT_MIN_HP = 0.0,  -- duration mult for a harbor that was reduced to 0 hp

	FLOATING_HARBOR_MIN_DECAY = 0.1, -- Always reduce Floating Harbor longevity by this many "hours" per hour
	FLOATING_HARBOR_DECAY_MAX_AIR_BONUS = -0.1, -- At 100% Friendly Air superiourity, change decay rate by this many "hours" per hour
	FLOATING_HARBOR_DECAY_MAX_AIR_PENALTY = 0.3, -- At 100% Enemy Air superiourity, change decay rate by this many "hours" per hour
	...
	SUPPLY_HUB_FULL_MOTORIZATION_TRUCK_COST = 65.0,
	...
	SUPPLY_HUB_MOTORIZATION_MARGINAL_EFFECT_DECAY = 1.8,


	-- used for calculating "flow" for railways.
	RAILWAY_BASE_FLOW = 6.5, 		-- how much base flow railway gives when a node connected to its capital/a naval node by a railway
	RAILWAY_FLOW_PER_LEVEL = 6.5, 	-- how much additional flow a railway level gives
	...
	INFRA_TO_SUPPLY = 0.95,							-- each level of infra gives this many supply
	...
	SUPPLY_POINTS_PER_TRAIN = 1.5,  -- old default 1.25 -- Amount of supply that can fit in a train. (Trains distribute supply from capital to a supply node.)
	...
	TRUCK_ATTRITION_FACTOR = 0.45, --a scale on total truck attrition
	...
	DAYS_TO_START_GIVING_FULL_SUPPLY_AFTER_MOVING_SUPPLY_CAPITAL =  1, -- the country will reach max supply after this many days moving its capital
	...
	SUPPLY_PATH_MAX_DISTANCE = 8,	-- max time it can take
	...
	SUPPLY_THRESHOLD_FOR_ARMY_ATTRITION = 0.4, -- armies will only get attrition below this supply
...
NIndustrialOrganisation = {
	...
	FUNDS_FOR_SIZE_UP = 1410064,									-- Funds needed for a MIO to increment its size and get points to unlock traits
	FUNDS_FOR_SIZE_UP_LEVEL_FACTOR = 0, 						-- How much each level mutliplies the funds for size up 
	...
	FUNDS_FOR_RESEARCH_COMPLETION_PER_RESEARCH_COST = 0,     	-- Funds added to MIO when the Design Team has completed a research, multiplied by research_cost in technology template
	...
	FUNDS_FROM_MANUFACTURER_PER_IC_PER_DAY = 0.0,				-- Funds added to MIO when a manufacturer is attached to a production line. Added every day proportional to IC produced.
	MAX_FUNDS_FROM_MANUFACTURER_PER_DAY = 0,					-- Max funds generated per manufacturer per day. Set to 0 for no Maximum.
	DESIGN_TEAM_RESEARCH_BONUS = 0.15,							-- Research bonus for applying a Design Team that matches the technology
	...
	DEFAULT_INITIAL_POLICY_ATTACH_COST = 9999,					-- Default start attach cost in PP for policies
